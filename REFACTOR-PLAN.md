...
- 🔨 **Phase 4 (type safety) — enum dispatch, in progress** (behavior-preserving; each increment gated: clippy 0/0, app+state tests, selftest 55/55, goldens 17/17):
  ...
  - ⏭️ **Remaining:** newtypes for the packed df32/span arrays (`ref_offset`, `span_mantissa`) so a transposition is a type error; `thiserror` error enums (`GpuError`/`ExportError`/`AppError`) replacing `Result<T, String>` + typed export `read_*`; surface swallowed `let _ = fs::write(…)`.
