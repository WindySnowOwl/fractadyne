...
- 🚧 **Phase 2a — field grouping (7 groups so far).** Breaking up the ~165-field `FractadyneApp` god-struct one cohesive group per gated commit: `GotoDialog`, `ShareDialog`, `BenchConfig`, `GalleryState`, `NavHistory`, **`ExportState`** (14 fields, 5 files — the churniest), **`EffectsConfig`** (lighting/glow, persisted). The pattern is proven across every variation (transient + persisted, method-name collisions, multi-line chains, `app.` bindings, `SessionState` mapping). *Remaining: the other persisted config groups (coloring, animation, autopilot) and scattered dialog flags; then Phase 3 moves the `draw_*` methods into a real `fractadyne-ui` crate.*
...
- 🔨 **Phase 4 (type safety) — enum dispatch, in progress** (behavior-preserving; each increment gated: clippy 0/0, app+state tests, selftest 55/55, goldens 17/17):
  ...
  - ⏭️ **Structured errors — remaining (slices 5–6):** `GpuError` (gpu `render_*`; the load-bearing cross-thread `Err(e) if e == "canceled"` → a `Canceled` variant) and `AppError` (app; `#[from]` Gpu/Export/Io + `Parse`/`Message`), replacing the ~5 remaining `Result<_,String>` + removing the temporary `.map_err(to_string)` boundaries in `export.rs`. Then surface the two user-action saves that silently `let _ = fs::write(…)` (benchmark Save, bookmarks) via a toast. Phase 5 touches cancel semantics across a worker thread — treat carefully + adversarially verify.
