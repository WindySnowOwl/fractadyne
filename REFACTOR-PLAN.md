...
- ✅ **Phase 2b — `update()` dialog + panel extraction (in progress, big wins).** All **10 modal dialogs** (goto, share, reset, toast, bookmarks, 3× benchmark, gallery, export) plus the **menu bar** (`draw_menu_bar`) and the **right control panel** (`draw_right_panel`) extracted from the ~2300-line `update()` into methods on a dedicated `impl FractadyneApp` block. **`update()` is now ~750 lines** — what remains is setup/background-polling, the status bar, the central render, and the perf/nav tail. Large blocks were moved *mechanically* (verbatim `sed` moves — no transcription risk). Gated each step (selftest 55/55, goldens 17/17, clippy 0). *Remaining Phase 2: status bar + central render out of `update()`, then the **field-grouping** (2a) of the ~165-field struct — which unblocks pulling the dialog/panel methods into a real `fractadyne-ui` crate.*
