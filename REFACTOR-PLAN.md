...
- ✅ **ARCHITECTURE.md** brought current with the refactor (core submodules, formula table/constants/checklist, 17-golden coverage).
- ⏭️ **Remaining Phase 1: the `gpu` module split (1b)** — `gpu/lib.rs` (~1370 lines) into `uniforms`/`pipeline`/`live`/`export` submodules, same mechanical pattern as the core split.
