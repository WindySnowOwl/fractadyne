...
- 🔨 **Phase 4 (type safety) — enum dispatch, in progress** (behavior-preserving; each increment gated: clippy 0/0, app+state tests, selftest 55/55, goldens 17/17):
  ...
  - ⏭️ **Structured errors — remaining (slice 6):** surface the two user-action saves that silently `let _ = fs::write(…)` — benchmark **Save** (toast the write result) and **bookmarks** (needs a `ctx`/pending-toast thread through `add_bookmark`). Plus optional: adopt `AppError` in `load_kfr_file` / `load_regions` (adding the `Parse`/`Io` variants) to retire the last app `Result<_,String>` sites.
