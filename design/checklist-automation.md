...
`validation/release-checklist.xlsx` (generated by `scripts/release_checklist.py`) is 124 manual
...
**All seven phases are implemented. 0 rows outstanding**: of 124 steps, 81 fully enforced, 34
partly, 7 that cannot be, and 2 that are process rather than product. Run
...
### The Linux block (added 2026-09-04)

Steps 97–106 are a **`Linux` area**, to be marked N-A on a Windows review. Linux has been a
...
|---|---|---|
| 97 | Profile lives at `~/.config/Fractadyne` (`ProjectDirs`, `$XDG_CONFIG_HOME`), not beside the binary | `test:config_lives_in_the_user_profile` |
| 98 | `--selftest` from the tarball uses the **cross-GPU** golden tolerance, proving `BLESSED-GPU.txt` shipped | `harness:--selftest` |
| 99 | ⭐The **no-`TIMESTAMP_QUERY`** path for real: absent on GL and several Mesa/RADV/ANV combinations, and until now only ever faked on NVIDIA via `FRACTADYNE_NO_TIMESTAMPS=1` | `selftest:live-res` |
| 100 | Each swappable GPU: adapter, backend, deep-view correctness (the 3080-black / 3070-rich bug is open) | *manual* — the hardware |
| 101 | Wayland **and** X11 | *manual* — two session types |
| 102 | ⭐The four `/proc` + sysfs diagnostics (RSS, statvfs, cpuinfo, VRAM) report plausible values — they were verified only to COMPILE | `uitest:diagnostics-populated` |
| 103 | Native GTK file dialogs | *manual* — a native dialog |
| 104 | No audio server: the finish tone must not be able to take the app down | `harness:--no-sound` |
| 105 | `--version` over SSH with no `DISPLAY` | `partial:test:title_string_matches_version` |
| 106 | `~/.config/Fractadyne/logs/` holds no new `crash-*.txt` | `uitest:no-crash-files` |
