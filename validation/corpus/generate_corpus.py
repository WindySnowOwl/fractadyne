...
<section class="card" id="{slug}">
  ...
  <div class="pair">
    ...
    <figure>
      ...
      <figcaption>Kalles Fraktaler &mdash; render <code>locations/{slug}.kfr</code> at {w}&times;{h}
        and save as <code>renders/{slug}-fraktaler.png</code></figcaption>
    </figure>
  </div>
  <table class="meta">
...
center / magnification / iteration cap for side-by-side structural comparison. Fractadyne renders are
generated by <code>generate_corpus.py</code>; produce each Fraktaler counterpart by opening the
<code>.kfr</code> in Kalles Fraktaler 2 (or fraktaler-3), rendering at {w}&times;{h}, and saving the PNG
into <code>renders/</code> with the <code>-fraktaler</code> suffix.</p>
