...
## Open-items index (116 open — regenerated 2026-09-17)
...
|-----|----|------|---------|------|
| red | 2 | Deep full-quality export: fixed per-dispatch cost + a sustained-load device loss … | Open bugs | 860 |
| red | 2 | FIELD DEVICE LOSS 2026-08-22, USER'S OWN DEEP ZOOM, beta.129 build 1837, RTX 3080, … | Open bugs | 2471 |
| red |  | "Normalize deep colors" GETS NO READINGS AT FLOATEXP DEPTH, so it cannot engage on load … | Performance & throughput (M7) | 9258 |
| red |  | `--uitest` step 43 `minimap-pan-verify` is a TIMING RACE, not a flake to wave through … | Performance & throughput (M7) | 9287 |
| orange |  | MOVING THE WINDOW BETWEEN MONITORS WITH DIFFERENT SCALING RESIZES IT ERRATICALLY AND … | Open bugs | 1174 |
| orange |  | DUAL-VIEW JULIA SOMETIMES STAYS BLURRY OR LANDS ON A FLAT PLANE AFTER MOTION … | Open bugs | 2447 |
| orange |  | DEVICE LOSS in `--motiontest` at Home→Settle, MODE 1 (direct), NOT reproducible — one … | Open bugs | 3494 |
| yellow |  | Relative-performance benchmark kit — F3 + Imagina + FractalShark, public (user, … | Proposed feature set — reconciled backlog (2026-08-06) | 8476 |
| green |  | Offline render: order frames by DECREASING MAGNIFICATION and reuse one reference across … | Open bugs | 3622 |
| green |  | Peer-renderer adoption list — reconciled (2026-08-18). Sources: `5E-324/Imagina` (+ Imagina 2), … | Open bugs | 3651 |
| - | 2 | ▶⭐⭐PARAMETRIC GRADIENT EDITING — P1, P2 and P3′ SHIPPED; ▶P4′ (Bézier kind 5) IS NEXT. | Open bugs | 883 |
| - | 2 | ▶⭐⭐PALETTE IMPORT — THE PLANNED ORDER IS COMPLETE: LUT + segment model (beta.23), `.map` (beta.24), `.ugr`… | Open bugs | 1027 |
| - | 2 | T2c. Restructure the acceleration table along MipLA lines — the converged recommendation of … | Open bugs | 3734 |
| - | 2 | T3. Julia-mipmap early termination (Imagina's renormalization trick): once a pixel's tail … | Open bugs | 3778 |
| - | 2 | The shader compiler FOLDS the df32 error-free transforms — "df32" has been ~f32 on … | Open bugs | 5274 |
| - | 1 | T2a. Derive BLA level-0 in-shader. `a = 2Z_n`, `b = one`, `r = |2Z_n|·eps`, `span = 1`, all … | Open bugs | 3722 |
| - | 1 | Runtime EFT / fused-fma capability probe that derates mode thresholds. Buried in a … | Open bugs | 3792 |
| - | 1 | Attempt a Fractadyne render of an ultradeep minibrot shared by the FractalShark author. | ▶ Render the ultradeep view34 location (someday) (2026-09-12) | 9326 |

### Unflagged backlog (98)

| Item | Section | Line |
|------|---------|------|
| C1 — in-flight tile ring for large multi-tile exports (submit 2–3 tiles, map whichever … | ▶ Image rendering path rework (2026-09-15) | 316 |
| C3 — GPU work arbiter: give live rendering a latency budget and export a bounded share … | ▶ Image rendering path rework (2026-09-15) | 321 |
| D3 — cache I/O outside the `STORE` mutex (`refcache_persist.rs offer`): reserve under the … | ▶ Image rendering path rework (2026-09-15) | 324 |
| E (chunking-coverage half): resumable state for auxiliary/non-holomorphic paths where … | ▶ Image rendering path rework (2026-09-15) | 328 |
| D — B3 async export-prep was assessed as very high blast-radius (`build_export_request` is … | ▶ Image rendering path rework (2026-09-15) | 332 |
| 📣ANNOUNCE DRAFTS ARE READY AND LIVE ONLY IN THE GITIGNORED `local/` FOLDER (2026-08-26). | Open bugs | 2195 |
| Cache the export `ShaderModule` per device (split out 2026-09-09 from the closed … | Open bugs | 3439 |
| T1b. Chebyshev norms in the hot radius tests (`max(|re|,|im|)` for `fe_abs_sf`). ⚠Reviews … | Open bugs | 3714 |
| T2b. Per-level precision demotion (Imagina's `ConvertStageToDouble` transposed): levels whose … | Open bugs | 3730 |
| T2d. Self-referential orbit compression for `refcache_persist`. Imagina's scheme (perturb the … | Open bugs | 3765 |
| Exponential-map EXR export for the zoomasm pipeline — low effort, and it plugs into the … | Open bugs | 3799 |
| Informational: watch `ImaginaFractal`'s module ABI. If Imagina 2 lands as a platform, interop … | Open bugs | 3801 |
| The deep LIVE view is strongly hardware/OS-dependent — same view, very different render … | Open bugs | 5520 |
| WATCH — was "PRIORITY", re-labelled 2026-09-09: unexplained access violation, `08-07 17:00:25 … | Open bugs | 6297 |
| CSV point list → generated tour (user, 2026-08-13): import a set of points from a CSV … | Script format — requested extensions (user, 2026-08-09) | 6793 |
| Tour files should be able to set every rendering parameter the UI can. Today `[render]` … | Script format — requested extensions (user, 2026-08-09) | 6834 |
| Tour-driven debug logging for remote diagnosis. The ability to annotate a tour with … | Script format — requested extensions (user, 2026-08-09) | 6842 |
| Target selection: score candidates, don't average. Use Newton nucleus finding to locate … | Autopilot / auto-zoom — target selection & path planning (user spec, 2026-08-07) | 7174 |
| Aim beside the nucleus, not at it. Landing exactly on a minibrot terminates the zoom in a … | Autopilot / auto-zoom — target selection & path planning (user spec, 2026-08-07) | 7181 |
| Prefer Misiurewicz targets as the other good class. λ guarantees the descent never runs … | Autopilot / auto-zoom — target selection & path planning (user spec, 2026-08-07) | 7184 |
| Path planning: receding horizon with commitment. Pick a target several decades ahead, … | Autopilot / auto-zoom — target selection & path planning (user spec, 2026-08-07) | 7186 |
| Fly it with Van Wijk & Nuij (2003), "Smooth and efficient zooming and panning" — the … | Autopilot / auto-zoom — target selection & path planning (user spec, 2026-08-07) | 7189 |
| Two cautions specific to this stack: … | Autopilot / auto-zoom — target selection & path planning (user spec, 2026-08-07) | 7195 |
| CA 2-D Birth/Survive rows only go 0–5; must be 0–8. A 2-D life-like cell … | Design follow-ups (from mockup review, 2026-06-25) | 7204 |
| Mockup `12` footer says "1 rule" but shows 2 rows (the 2nd, `G →`, is an … | Design follow-ups (from mockup review, 2026-06-25) | 7207 |
| Smarter exterior sampling — adaptive/jittered supersampling or higher … | Milestone M2 — Deep zoom (in progress) | 7476 |
| Coloring tuning — optionally scale color cycling with zoom so steep … | Milestone M2 — Deep zoom (in progress) | 7478 |
| L-systems, cellular automata (1-D & 2-D). | Milestone M3/M4 — Fractal variety & dual view (in progress) | 7600 |
| Tour editor (visual timeline) — an in-app editor for camera tours: a scrubbing timeline … | Tooling, scripting & versioning (M7) | 7733 |
| Benchmark presets — multiple scenes (Julia deep, Multibrot, dual) + CSV/JSON … | Tooling, scripting & versioning (M7) | 7753 |
| In-app editor for the authorable files — a text/TOML editor (probably a Tools → … | Tooling, scripting & versioning (M7) | 7802 |
| Theme polish — optional light/preset themes, custom font, accent picker. | Branding & UI (M7) | 7862 |
| Navigable minimap (user, 2026-08-09) — dragging (or clicking) the "you are here" … | Branding & UI (M7) | 7863 |
| Internationalization (user, 2026-08-09) — externalize user-facing strings and support … | Branding & UI (M7) | 7872 |
| Layers + blend modes (Ultra Fractal-style compositing). | Survey-driven roadmap (2026-06-28) | 7972 |
| Formula DSL / custom formulas (M6). | Survey-driven roadmap (2026-06-28) | 7973 |
| 3D fractals (Mandelbulb / Mandelbox, ray-marched). | Survey-driven roadmap (2026-06-28) | 8072 |
| Flame / IFS fractals; L-systems; cellular automata. | Survey-driven roadmap (2026-06-28) | 8073 |
| Adopt `egui_kittest` (egui's official test harness, same 0.31 workspace as our pinned … | UI test automation | 8082 |
| Shallow-view resize pacing — the direct-mode path re-iterates + reallocates the … | UI test automation | 8096 |
| Custom formula / coloring scripting — Ultra Fractal's killer feature (user-written … | Feature gaps vs. peer renderers (survey 2026-08-05: KF2 / Fraktaler-3 / Ultra Fractal / XaoS / Imagina) | 8118 |
| Huge tiled exports — KF renders wall-sized images by tiling; we cap at the GPU texture … | Feature gaps vs. peer renderers (survey 2026-08-05: KF2 / Fraktaler-3 / Ultra Fractal / XaoS / Imagina) | 8123 |
| Acceleration breadth — SA covers Mandelbrot + Multibrot 3–5 only; Fraktaler-3's … | Feature gaps vs. peer renderers (survey 2026-08-05: KF2 / Fraktaler-3 / Ultra Fractal / XaoS / Imagina) | 8126 |
| Glitch correction robustness at depth — KF's Pauldelbrot correction is mature; ours has … | Feature gaps vs. peer renderers (survey 2026-08-05: KF2 / Fraktaler-3 / Ultra Fractal / XaoS / Imagina) | 8129 |
| Ecosystem/platform — no plugin system; ~~Windows-only release builds~~ Windows + Linux … | Feature gaps vs. peer renderers (survey 2026-08-05: KF2 / Fraktaler-3 / Ultra Fractal / XaoS / Imagina) | 8131 |
| WYSIWYG coordinate-expression editor (user, 2026-09-11). The Go-to fields take the … | Proposed feature set — reconciled backlog (2026-08-06) | 8204 |
| Newton-Raphson zoom follow-ups — (a) run the refine off-thread; a period-100k atom at … | Proposed feature set — reconciled backlog (2026-08-06) | 8242 |
| Landmark library expansion (P1, §4.3 partial) — we ship 12 curated points; add the … | Proposed feature set — reconciled backlog (2026-08-06) | 8253 |
| Automated tour-frame verdict harness — render `grand-tour.toml` (low fps/size) and … | Proposed feature set — reconciled backlog (2026-08-06) | 8282 |
| Original grand-tour spec (superseded, kept for the unbuilt parts) — one tour that visits the known … | Proposed feature set — reconciled backlog (2026-08-06) | 8288 |
| Complete the G-buffer (P0, §1.1 partial) — the iterate target already carries smooth … | Proposed feature set — reconciled backlog (2026-08-06) | 8306 |
| Method switching without re-iterating (P0, §1.2 partial) — palette, cycle, offset and … | Proposed feature set — reconciled backlog (2026-08-06) | 8311 |
| Raw-channel EXR export (P1, §1.3 partial) — we write a linear *color* EXR with embedded … | Proposed feature set — reconciled backlog (2026-08-06) | 8316 |
| Exponential-map render mode (P1, §1.4 absent) — zoomasm-compatible zoom-video assembly. | Proposed feature set — reconciled backlog (2026-08-06) | 8320 |
| Resumable/checkpointed tiled export (P2, §1.5 partial) — tiled iterate exists; see … | Proposed feature set — reconciled backlog (2026-08-06) | 8321 |
| Interior distance-estimation coloring (P0, §2.2 gap) — exterior DE ships and is stable … | Proposed feature set — reconciled backlog (2026-08-06) | 8334 |
| Log-scaled + histogram/percentile palette mapping (P0, §2.4 partial) — `--normalize` and … | Proposed feature set — reconciled backlog (2026-08-06) | 8363 |
| Curvature-average coloring (P1, §2.5 gap) — stripe, triangle-inequality and orbit traps … | Proposed feature set — reconciled backlog (2026-08-06) | 8366 |
| Layer compositing (P1, §2.7 absent) — multiple colorings over one G-buffer with masks and … | Proposed feature set — reconciled backlog (2026-08-06) | 8374 |
| DE-adaptive anti-aliasing (P1, §2.8 absent) — supersample only where DE says the boundary … | Proposed feature set — reconciled backlog (2026-08-06) | 8376 |
| User-scriptable coloring (P2, §2.9 absent) — a WGSL snippet slot over the G-buffer; see … | Proposed feature set — reconciled backlog (2026-08-06) | 8378 |
| Interior colorings (P2, §2.10 absent) — period coloring, atom domains, interior-coordinate … | Proposed feature set — reconciled backlog (2026-08-06) | 8380 |
| (2) Bifurcation-measure coloring mode — μ_bif = Δ G(c), and the G-buffer already holds … | Proposed feature set — reconciled backlog (2026-08-06) | 8390 |
| (1) Galois orbits of PCF parameters — the flagship. For fixed (k,n) the PCF parameters … | Proposed feature set — reconciled backlog (2026-08-06) | 8398 |
| (4) Unlikely-intersection explorer (DeMarco–Krieger–Ye) — two parameters c₁, c₂; … | Proposed feature set — reconciled backlog (2026-08-06) | 8411 |
| (3) Exact rational critical-orbit arithmetic — for c ∈ ℚ iterate the critical orbit in … | Proposed feature set — reconciled backlog (2026-08-06) | 8418 |
| (5) Special curves / PCF loci in two-parameter families (André–Oort setting) — the … | Proposed feature set — reconciled backlog (2026-08-06) | 8429 |
| Embedded-Julia size/orientation estimates (P1, §3.5 absent) — shape-stacking navigation; … | Proposed feature set — reconciled backlog (2026-08-06) | 8448 |
| Orbit cycle detection + multipliers (P1, §3.6 partial) — the cursor-point orbit plot, … | Proposed feature set — reconciled backlog (2026-08-06) | 8450 |
| Critical-orbit overlay in the Julia panel (P2, §3.7 gap) — the dual linked view otherwise … | Proposed feature set — reconciled backlog (2026-08-06) | 8453 |
| Guided descent modes (P1, §4.4 partial) — approach-nucleus, λ-stepped Misiurewicz … | Proposed feature set — reconciled backlog (2026-08-06) | 8456 |
| External angles and rays (P2, §3.8 absent) — landing angles, parameter rays, equipotentials. | Proposed feature set — reconciled backlog (2026-08-06) | 8459 |
| Internal addresses and tuning navigation (P2, §3.9 absent) — "go to p/q bulb", display the … | Proposed feature set — reconciled backlog (2026-08-06) | 8460 |
| Verified computation mode (P2, §3.10 absent) — interval/ball arithmetic for certified … | Proposed feature set — reconciled backlog (2026-08-06) | 8462 |
| Numeric egress (P2, §3.11 absent) — iteration grids, orbits, ray data as CSV/NumPy via the … | Proposed feature set — reconciled backlog (2026-08-06) | 8464 |
| Auto-stretch (P2, §4.5 absent) — unskew sheared deep locations from iteration histograms … | Proposed feature set — reconciled backlog (2026-08-06) | 8469 |
| `.kfr` export / `.kfb` support (§4.2 gap) — `.kfr` import ships (hardened + fuzzed); … | Proposed feature set — reconciled backlog (2026-08-06) | 8471 |
| F3 goldens for the tours' key steps (user, 2026-08-10). The grand tour's regression … | Proposed feature set — reconciled backlog (2026-08-06) | 8842 |
| Tan Lei invariant goldens (P0, §5.1 absent) — at a Misiurewicz landmark, render at … | Proposed feature set — reconciled backlog (2026-08-06) | 8855 |
| Landmark benchmark regimes (P1, §5.2 partial) — `--bench-matrix` covers zoom bands, … | Proposed feature set — reconciled backlog (2026-08-06) | 8860 |
| λ-scaling zoom-loop test (P1, §5.3 absent) — a Misiurewicz-centered zoom must become … | Proposed feature set — reconciled backlog (2026-08-06) | 8864 |
| Per-channel cross-tool differential testing (P2, §5.4 partial) — the F3 corpus (20 … | Proposed feature set — reconciled backlog (2026-08-06) | 8866 |
| Periodic references for nucleus-centred views — the autopilot/NR-zoom deliberately … | Performance & throughput (M7) | 8883 |
| One reference per zoom sequence (offline tours) — a zoom toward a fixed centre uses … | Performance & throughput (M7) | 8891 |
| Level 1: three-thread complex squaring — (x+iy)² = three independent bignum mults (or … | Performance & throughput (M7) | 8896 |
| Level 3 (research): precision-cascade with parallel residuals — run the serial chain at … | Performance & throughput (M7) | 8913 |
| Precision ramp-down along the orbit (research) — later iterations may tolerate less … | Performance & throughput (M7) | 8919 |
| By-frame render farm = a documented script, not a subsystem. Frames are independent and … | Performance & throughput (M7) | 8930 |
| By-keyframe distribution once exponential-map export lands — one keyframe per factor … | Performance & throughput (M7) | 8964 |
| Shippable reference files — the head node computes the sequence reference once and … | Performance & throughput (M7) | 8967 |
| Distributed normalize coherence — per-machine `--normalize` ranges flicker at shard … | Performance & throughput (M7) | 8973 |
| GPU-assisted reference-candidate scoring (the practical "bignum on GPU"). Full GPU bignum … | Performance & throughput (M7) | 8981 |
| Deep floatexp *settled* frames are slow in filament fields — a shader-speed fix, NOT multi-reference. | Performance & throughput (M7) | 9013 |
| Tile-level export pipeline — overlap tile N+1 iterate with tile N async readback + … | Performance & throughput (M7) | 9110 |
| Better single-GPU utilization — before adding GPUs, check the live dispatch actually … | Performance & throughput (M7) | 9123 |
| `about_names_the_running_backend` is not isolated from the rest of its test binary — … | Performance & throughput (M7) | 9248 |
| Multi-GPU — offline/export only (deferred) — a second GPU gives near-linear speedup for … | Performance & throughput (M7) | 9300 |
| Multi-GPU — offline/export only (deferred) — a second GPU gives near-linear speedup for … | Performance & throughput (M7) | 9303 |
<!-- TODO-INDEX:END -->
...
13. **Script playback is buried and forgets what you played.** Two small UX fixes to the feature
    ...
    *Tools → Play script…* (`ui/menus.rs`, `load_script()`), which opens a file dialog every time:
    ...
    - [x] ✅**SHIPPED v0.2.40-beta.76 (stage A, the zoom) + v0.2.40-beta.78 (stage B, the settle is
      present-gated too) — the entry was never checked; found stale 2026-09-09.** Original request:
...
- [x] ✅⭐⭐**FIXED 2026-09-17 — only an ESCAPED reference seeds the boost, and the `--uitest` live
  capture now waits for the pixel walk.** `boost_seed_from_reference(ref_len, cap, escaped)`
  returns no seed unless the orbit ended on its own (`orbit_tail.escaped && !partial`); a
  non-escaping orbit stops at the length we asked for, so it is not evidence. The capture gate
  gained `&& !settling`, the same quiet period every other step uses (the walk's flags were
  already in `busy` — the live arm just ignored them).
  ✅**Measured, same step, same view:** `eff_iter` **1,004,384 → 18,181**, frame **14.6 ms flat
  brown → 1.6 ms** of rich valley (`stddev 0.0, 1 bucket` → `68.9, 31/32`). The other live bands
  came down with it and stayed correct: 1e12 1,008,769 → 12,204, 1e30 → 44,019.
  ✅**Gates: `--uitest` 45/45 — the first clean walk in weeks** (44/45 since at least beta.106),
  livetest 24/24 with **0 drift** (it grades live output against an OFFLINE render at every
  checkpoint, so a starved deep view would show as excess blackness — none did), motiontest PASS,
  autodive 1e32 with 0 lethal readings, selftest 189/190 + goldens 19/19 (the unrelated
  deep-minibrot framing row), 393 app tests including
  `only_an_escaped_reference_seeds_the_boost`, which pins both the 1e6 landmark (no seed) and the
  6.3e63 spar (still seeds). Original finding follows.

  ⛔**THE ADAPTIVE ITERATION BUDGET SEEDED ITSELF FROM A NUMBER THAT MEASURES NOTHING —
  a shallow view ran 140× the iterations it needed and showed a FLAT frame while it ground
  (verified 2026-09-17).** This is what `--uitest` step 30 (`live-df32-1e6`) had been failing on
  every run, on every build **including the released beta.106** — it had been reported as "the
  known pre-existing failure" for weeks and was a real defect, not a harness artefact.
  ⭐**The measurement.** The step jumps the live view to the canonical Seahorse point
  (`-0.743643887037158704752191506114774 + 0.131825904205311970493132056385139i`) at **1e6×** on
  the adaptive budget. The app settles on **`eff_iter` 1,004,384** with `orbit_len` 1,012,577 and
  paints one flat brown (`stddev 0.0, 1 luma bucket`) in a 14.6 ms frame. The OFFLINE renderer at
  the same view and the same million-iteration budget draws a rich valley with a minibrot — and
  so does the same view at **5,000** iterations, *visually identical*. So the budget is ~140× the
  demand; the offline frame costs **139 s at 640×400** where 5,000 iterations costs 0.2 s.
  ⭐**The mechanism, and it is a one-line reading.** `boost_seed_from_reference(ref_len, cap) =
  clamp(ref_len / cap, 1, ITER_BOOST_MAX)` seeds the boost from the REFERENCE ORBIT'S LENGTH, on
  the argument that "no pixel can outrun the orbit". True — but a reference whose centre does not
  ESCAPE has a length equal to the build target, not to anything the pixels need, and a deep-zoom
  landmark centre is by construction on the boundary. Here `zoom_iter_cap(19.93) = 2000 + 19.93 ×
  256 = 7,102` and the seed lands at `1,012,577 / 7,102 = 142.6` (under the 256 ceiling), so
  **budget = 7,102 × 142.6 ≈ the reference length itself**: 1,004,384 ≈ 1,012,577. The two
  numbers agreeing to 1% IS the bug.
  ⛔**And it is self-sustaining**: the reference is built to serve the budget
  (`ref_build_iter = gpu_iter.max(eff_iter) + 32·256`), so a big budget builds a long reference,
  which seeds a big budget. The seed reads a value that is DOWNSTREAM of the thing it is sizing —
  the same shape as "a gate read a DERIVED value; compare RECORDED INPUTS" (beta.130).
  ⭐**Fix direction** (not attempted — this wants its own measurement pass): seed only from a
  reference that ESCAPED, where the length is genuine evidence of demand (`orbit_tail.is_none()`
  marks escape; `partial` marks truncation). A non-escaping orbit's length is the cap and must
  seed nothing. ⚠The seed exists for a real case — a settled deep view capping pixels at 69k with
  a 256k reference — so the guard must keep that one working; the 1e55 spar view is the fence.
  ⚠**The `--uitest` gate is ALSO too generous here**: the live step captures once the REFERENCE is
  quiet (`StepKind::Live(v) => … || ref_settled`, 30 s hard cap), which says nothing about whether
  the chunked pixel walk has finished. Even with a sane budget the gate should wait for the walk,
  or the check will keep grading a half-drawn frame.
  ⭐Reproduce: `--uitest DIR` step 30, or the oracle pair
  `--render -o a.png --center <cx> <cy> --zoom 1e6 --iter 5000` vs `--iter 1004384`.
...
- [ ] ▶⭐⭐**PARAMETRIC GRADIENT EDITING — P1, P2 and P3′ SHIPPED; ▶P4′ (Bézier kind 5) IS NEXT.**
  ...
  ⭐⭐**The finding that sizes the job: the ENGINE IS ALREADY PARAMETRIC, the EDITOR IS NOT.**
  Per-segment blend curve, colour space and midpoint already evaluate, bake to the LUT, persist
  (`PaletteSegment`) and are gated end to end by `.ggr` import — but the only way a user reaches any
  of it is importing a `.ggr` someone else authored. The editor edits `[pos,r,g,b]` and CLEARS
  `custom_segments` on every edit. This is UI on a finished engine plus one data-model decision.

  **DECIDED (user, 2026-09-05)**: take the FULL segment-native editor, and go beyond GIMP's five.
  ⭐⭐**Flexibility comes from PARAMETERS, not more names** — 27 extra named curves is a worse
  editor. So **32 is numbering space, deliberately under-populated**: kinds **0–4 GIMP (FROZEN, a
  file format in `.ggr` AND in our sessions)**, **5 = cubic-Bézier easing** (4 floats, the CSS
  model — subsumes power/gamma, adds S-curves, off-the-shelf widget), **6–31 reserved and empty**.
  A single "bias" slider + presets drive the Bézier handles, so there is ONE storage shape and two
  ways to drive it. **32 hand-editable stops** (up from `EDITOR_MAX_STOPS = 24`).

  **Phases, and P1 ships alone**:
  ✅**P1 DONE (2026-09-05) — ZERO DRIFT, MEASURED.** `custom_segments` is the source of truth; the
  editor's rows are a VIEW of segment boundaries and every edit goes through a segment operation
  (`set_stop_color` / `set_stop_position` / `insert_stop` / `remove_stop`) that preserves the blend,
  colour space and midpoint FRACTION of every segment it does not remove. Legacy sessions migrate
  on load; `custom_gradient`'s stop/band branches stay as a deliberate SAFETY NET for render-only
  paths (`--palette-*` exits before an editor exists).
  ⭐**Acceptance met in full**: four palette paths (`.map` bands, `.map` smoothed, `.ugr`, `.ggr`)
  rendered on the pre-P1 and P1 builds are **byte-identical, maxD 0, 0 differing px**; selftest
  **173/173 + 18/18**; F3 corpus **38/38 maxD 0**; **no re-bless**. `segment/edit_tests.rs` (8 new,
  color crate 54→62) pins the zero-drift contract directly — every op equals what `from_stops`
  would give for the edited stop list.
  ⚠**Two behaviours the migration would have broken, caught in review**: the *Hard bands* checkbox
  flipped a flag nothing read any more (UI responds, picture does not — the exact failure the
  source-of-truth change exists to end), and *Add stop* did nothing on a fresh session. Both fixed;
  Add stop now splits the WIDEST segment rather than a fixed 0.5, which cannot silently no-op.

  1. ✅**Segment-native model** — `custom_segments` becomes the source of truth, stops a derived view.
     ⚠The MIGRATION is the risky half, not the maths. Acceptance: an old stop-only session renders
     **byte-identically**, selftest 173/173 + 18/18, corpus 38/38, **NO re-bless**.
  2. ✅**DONE beta.29 — expose what exists.** One row per SEGMENT: blend combo, space combo,
     midpoint (as a FRACTION of the span, GIMP semantics), and a preview drawing the REAL curve via
     the newly exposed `Segment::factor` — so the preview cannot drift from what renders, and the
     two non-midpoint-symmetric sphere curves are drawn honestly. ⭐**The HSV-from-a-grey trap is
     surfaced**: `Segment::hue_undefined_endpoint` puts a ⚠ on any HSV segment with an unsaturated
     end, explaining that the sweep starts at red and crosses the whole wheel. A warning, not a
     restriction — the effect is legitimate. Also an empty-state hint, since a fresh session has no
     custom gradient and the section would otherwise be invisible with no explanation.
     ⭐**Zero drift held**: 4 palette paths byte-identical vs the PRE-P1 build (maxD 0), selftest
     173/173 + 18/18, workspace green (color 62→64). No re-bless.
     ⚠**The layout itself is unverified by machine** — no harness opens the gradient editor, so the
     row fits in the ~340 px window on the author's eye alone. P3 widens it.
  3. ✅**P3′ — THE LAYOUT REDESIGN — DONE beta.30.** `design/gradient-curves.md` §9. The user
     reviewed the shipped P2 editor and called it difficult to use; ⭐⭐**the diagnosis was that
     there is NO SELECTION, so everything must be on screen, so nothing can be big enough to
     read** (the curve preview compensating for undecodable blend names was **30×16 px**, and the
     ...
     every custom gradient; stale "positions may overlap" copy; the window's width set by whichever
     label was longest, because a label inside `ui.horizontal` never wraps), and ⚠⚠**the harness
     was lying by omission** — `--uitest`'s `Screen::PaletteEditor` step seeded NO custom gradient,
     so it screenshotted the empty state and passed; the one screenshot meant to review this
     surface never contained it. It also had the **Diagnostics window standing behind every
     screenshot from step 15 on**, invisible only because the window in front was wide enough.
     ⛔Per-channel R/G/B splines (Photoshop *Curves*) declined for a new reason: they make free
     control points the primary object, so **stops stop being the model** and every importer's
     round trip breaks. Survives as a read-only feedback plot — P6, not scope.
  3b. ✅**P3″ — WHAT USING IT ASKED FOR — DONE beta.31.** `design/gradient-curves.md` §9.6. The
     author ran beta.30 and asked for four things; all four shipped, with the bug report attached.
     ⭐⭐**"Drag points sometimes hang up where they stop moving" was real**: `drag_started()` fires
     only AFTER the pointer passes egui's threshold, so hit-testing `interact_pointer_pos()` there
     tests a point already several px from the press — now the **press origin**. ⚠The first
     diagnosis blamed the 6 px threshold and **the test's own guard proved it wrong**: 6 px does not
     leave a 9 px radius at 15.8 px spacing. The threshold is a LOWER bound; the drift is a whole
     frame of pointer motion, unbounded. ⭐The silent-cancel case needs FEW stops (at the 32 cap the
     catch zones overlap everywhere, so a drift grabs the wrong marker rather than none).
     Shipped: **Ring view** (arc-length hit-testing that WRAPS; the seam is the whole point and the
     bar cannot show it) · **an add lane whose LINE is the click target** with a `⊕` for the widest
  ...
  4c. ✅**SCREEN EYEDROPPER — DONE beta.34** (`eyedropper.rs`). A **Pick** button beside the stop's
     colour samples any pixel on the DESKTOP, not just our window — the colours people want come
     from a photo or another program. Raw Win32 FFI following `sysinfo.rs`
     (`GetCursorPos`/`GetDC(0)`/`GetPixel` + `GetAsyncKeyState`, because while picking the pointer
     is over another app and egui sees no input at all).
     ⭐⭐**`armed` is why it is a state machine**: the click that STARTS the pick is still down on
     the first poll, so "commit while down" grabs the colour of the dropper button itself.
     ⭐⭐**A `COLORREF` is `0x00bbggrr`** — the same BGR trap as `.ugr`, pinned with red/blue/green
     and an asymmetric value. ⚠`CLR_INVALID` is otherwise a legal-looking white, so an unsampleable
     click CANCELS rather than painting the stop white. ⭐**The full-screen overlay is the INPUT
     GUARD**, not decoration: without it the OS click that takes the colour would ALSO reach
     whatever egui widget is underneath.
     ⚠⚠**Windows only and it SAYS SO** (disabled + reason elsewhere): X11 needs a dependency,
     Wayland needs a portal. ⚠A sample costs **~8 ms** (measured), nearly all in GetDC/ReleaseDC —
     fine polled once a frame in a transient mode, and the reason it never samples outside one.
     ⭐The DC-leak test is the one thing about the FFI a machine can check: `GetDC(NULL)` hands back
     a COMMON DC from a cache a few deep, so a missing release shows within tens of calls.
  4b. ~~P4′ — Bézier kind 5~~ + the handles going live. First phase that may move pixels, only where
     ...
     allow y overshoot and clamp at the COLOUR not the factor. ⭐The `x(u)=t` solve is BAKE-TIME
     only (1024×/bake, 0×/pixel) — no solver in the shader.
  5. **`.ggr` export** + round-trip property test; kinds ≥5 approximate and MUST SAY SO.

  ⭐⭐**ZERO DRIFT IS THE ACCEPTANCE CRITERION for P1–P3**: `Linear`/`Rgb`/centred is the default and
  is exactly what `from_stops` already produces. If goldens or the corpus move, a DEFAULT moved —
  that is a bug, not a bless. (The last two palette changes each cost a re-bless conversation.)

  ⚠**Traps carried forward** (all grounded in §4 of the doc): ⭐⭐**HSV from a GREY sweeps the whole
  wheel** — `rgb_to_hsv` returns hue 0 for anything unsaturated and equal hues take a FULL turn, so
  a measured **black→red** segment is **GREEN in the middle** (21/65 samples green-dominant); the
  editor must show it · blend/space numbers are a FILE FORMAT, **append only** · midpoint is
  normalised WITHIN the segment · the two sphere curves are NOT midpoint-symmetric (0.866 at
  halfway) so a preview must draw the REAL function · ⚠⚠**no harness drives drag** ⇒ split the
  widget so hit-test/clamp/order/insert/delete are PURE tested functions · ⚠the LUT
  error-shrinks-with-size evidence was taken on LINEAR gradients — re-measure on a curve-heavy one.

  ⛔**Deferred with reasons**: OkLab (needs a real sRGB decode/encode round trip — the first place
  the display-referred choice costs something; its own decision) · per-channel curves (3× the
  parameters for a gain `cycle` erases) · alpha (renderer has no alpha channel).
- [ ] ▶⭐⭐**PALETTE IMPORT — THE PLANNED ORDER IS COMPLETE**: LUT + segment model (beta.23), `.map` (beta.24), `.ugr` (beta.25), `.ggr` (beta.26), `.ase` (beta.27). ⚠Two things remain OPEN, below: the §7 verification bar (no Fractint/UF render to compare against) and `.cs`, which is NOT implemented on purpose.** Design, format
  ...
  **`--palette-map FILE`** / **`--palette-map-smooth`** for scripted verification.
  ...
  ✅**SHIPPED beta.25**: `.ugr` import — `parse_ugr` → many named `UgrGradient`s, a picker in the
  editor with a preview swatch per row, and **`--palette-ugr FILE`** / **`--palette-ugr-name NAME`**
  (no name = first, and the run SAYS which; an unknown name exits 2 listing what the file holds).
  ⭐⭐**The BGR trap is verified AT THE PIXEL**: `color=` is `0xBBGGRR`, red in the LOW byte, and a
  render through a pure-red and a pure-blue gradient comes back red and blue — under an RGB reading
  they swap, and the result still looks like a plausible palette, which is how that bug survives a
  review. ⚠`rotation=` is APPLIED but its DIRECTION is unverified against UF itself (pinned by
  `ugr_rotation_shifts_the_ring`, so a correction is one sign change plus one test edit);
  `smooth=yes` is RECORDED but NOT honoured — UF's smooth mode is a spline through the control
  points, not a per-segment blend function, and gnofract4d imports linearly for the same reason.
  ⚠Our `/255` vs gnofract4d's `/256` means our import reaches pure white and theirs does not.

  ✅**SHIPPED beta.26**: `.ggr` import — **the only importer that loses NOTHING**, because a `.ggr`
  segment IS a model segment. Per-segment midpoint, blend curve and colour space all arrive
  verbatim. Editor **Import .ggr…** + **`--palette-ggr FILE`**.
  ⭐⭐**The HSV sweep is verified AT THE PIXEL with a one-integer control**: the same file rendered
  with colouring column `1` (HSV ccw) vs `0` (RGB), identical red endpoints — **810 distinct
  colours vs 4**. Both 13- and 15-column forms load; the `Name:` line is optional (GIMP 1.x).
  ⚠**A `+`-prefixed compressed continuation is REJECTED BY NAME, not guessed at** — §2 of the
  design doc mentions one but it was never confirmed against a real file or a working parser.
  ⭐⭐**This forced a NEW SESSION FIELD, `custom_segments`**, which WINS over `custom_palette` when
  set: a `[pos,r,g,b]` list cannot hold a midpoint, a blend curve or a hue sweep, so persisting a
  `.ggr` as stops would have flattened all three on the FIRST RESTART, silently. ⚠**Every path that
  sets stops must clear it** (presets, paste, `.map`, `.ugr`, add-stop) or those edits appear to do
  nothing — UI responds, picture does not. The editor refuses to pretend: a "Convert to editable
  stops" button states what it discards instead of flattening on the first click.

  ✅**SHIPPED beta.27**: `.ase` (Adobe swatch exchange) — big-endian binary, group markers
  flattened, RGB / Gray / naive CMYK. Evenly spaced + linearly blended, a choice WE make since a
  swatch list carries no positions. Editor **Import .ase…** + **`--palette-ase FILE`** (read as
  BYTES — `read_to_string` rejects it).
  ⚠⚠**UNVERIFIED AGAINST A REAL FILE** — the only importer written from the published layout rather
  than a real file or a working parser, because none was to hand. Deliberately STRICT as
  compensation (signature, block lengths and colour model must all agree) so a disagreeing file
  FAILS LOUDLY rather than importing plausible wrong colours. ⭐**The first real `.ase` anyone
  imports is the actual test.** ⚠The unit fixtures are BUILT FROM THE SAME UNDERSTANDING as the
  parser, so they prove consistency, not correctness — stated in the test module.
  ⚠**LAB is REFUSED, not converted** (needs a white point + adaptation the file does not carry); a
  MIXED file is refused too, because importing "the rest" would silently drop swatches.

  ⛔**`.cs` (ColorSchemer) is NOT implemented, on purpose.** It is a binary format I have no
  verified layout for, and guessing at one is exactly what the `.ggr` `+` continuation and the
  `.ase` LAB case were refused for. gnofract4d's `fract4d/gradient.py` is the reference if it is
  ever wanted; the product is long dead and the paste box already covers text swatch lists.

  ✅**SELFTEST COVERAGE ADDED beta.28** — the end-to-end checks were MANUAL until now, and a manual
  check is not a gate because it does not run again. `--selftest` is **170 → 173**:
  `map-bands-are-exact` (0 off-palette pixels; smoothed control has >3× the levels),
  `ugr-color-is-bgr` (each render's own channel leads by 2×, BOTH ways round),
  `ggr-colour-space-is-per-segment` (hue sweep yields >10× the distinct colours of the RGB
  reading). ⭐⭐**Each is a CONTROL PAIR** — the same file with ONE field changed — because the naive
  form of each cannot fail: "the .map render has few colours" is also true of a smoothed dark
  palette. ⭐**All three VERIFIED RED** by mutating their three mechanisms (flat flag off, BGR read
  as RGB, space forced to Rgb): 5→253 levels with 32,854 off-palette, r/b exactly swapped,
  1533→4 colours. ⚠`.ase` is NOT covered — its own format is unverified, so a render check would
  only confirm our parser agrees with itself.

  ✅**CHECKLIST ROWS ADDED (2026-09-05)** — the eyes-and-hands half, since a selftest cannot open a
  file dialog or judge a swatch. `release-checklist.xlsx` is **124 → 131 steps** (81 full / **38**
  partial / **12** manual+process). Steps **59–65**, all in Coloring: the editor's preview bar vs
  the RENDER (the beta.23 gamma fix removed the mechanism, but "same colour?" is still an eye
  judgement) · `.map` import + the Hard-bands toggle · ⭐the §7 **Fractint side-by-side**, `manual`,
  with BLOCKED a legitimate outcome · the `.ugr` picker · the `.ggr` "Convert to editable stops"
  flow · ⭐loading a **REAL `.ase`** (the format's actual test; a rejection is a result worth
  reporting). The four `partial:` claims resolve to the beta.28 selftest checks — verified by
  falsifying one and watching `checklist_coverage.py` exit 1.

  ⛔⭐⭐**FOUND WHILE DOING IT: `design/checklist-automation.md`'s STEP NUMBERS WERE ALREADY STALE
  — silently, for NINE of seventeen areas.** Steps had been appended to Locations, Tools, Help &
  settings and the rest, and a whole 10-step `Linux` area inserted at 97, without any heading or
  row number being moved. The per-row numbers stayed internally consistent, so nothing looked
  wrong from inside the document: every heading was plausible and pointed the reader at the wrong
  steps (Persistence rows said 107–109 under a heading that should have read 124–126). Fixed by
  recomputing every heading from the generator and rebasing each section's rows onto it (31 rows
  moved). ⭐**`checklist_coverage.py` now CHECKS BOTH** — heading ranges and each section's first /
  last / contiguity — and both were verified RED. It also reports the **8 steps that still have no
  row at all** (Locations 3, Accelerated build 4, Stability 1): a real pre-existing gap, now
...
- [ ] 📣**ANNOUNCE DRAFTS ARE READY AND LIVE ONLY IN THE GITIGNORED `local/` FOLDER (2026-08-26).**
  ...
  | file | words | state |
  |---|---|---|
  | `local/post-1-announcement.bbcode.txt` | ~1,300 | BBCode, fact-checked against `v0.2.41-beta.78` (2026-09-09: 183+18 self-test, 39-row corpus to 5.6e18003×, orbit cache, palette import, `.fdn` as a document, "not finished" rewritten); re-pointed the same day at `v0.2.41-beta.79`, the ONLY release left on GitHub (every earlier release incl. stable v0.2.40 withdrawn; tags kept) |
...
- [x] ✅**NORM READINGS ARE ATTRIBUTED AT DRAIN TIME — tag them with the SUBMITTING frame's
  ...
  ✅⭐⭐**DONE 2026-09-05 — the signature now travels WITH the reading.** `norm_signature()` is one
  definition used by both the submitting and the draining frame; the value rides
  `MandelbrotParams::norm_sig` → `CounterRead::norm_sig` (recorded at copy time, beside the `px`
  and `max_iter` that were already carried that way) → back on a `norm_sig_out` sink, stored in the
  SAME block of `pump` as the readings it describes. A settled frame whose reading carries a
  different signature DISCARDS it (`norm_reading_is_current`); motion accepts everything, because
  the view changes every frame and matching would starve the chase.

  ⭐⭐**MEASURED, and the first instrument was wrong.** An instrumented build over a real
  `--autodive 24`: **1,569 fresh readings, every one carrying a signature** (so the echo arrives —
  and had it not, the `read_sig == 0` fallback would have made the whole change a silent no-op),
  and **63% carried a signature different from the view on screen**. The discard fired twice, at a
  settle. ⚠**The FIRST run measured nothing and looked like proof of nothing**: `--livetest`'s
  continuous tour reprojects, and `arm_ctr` is inside `if !reproject`, so almost no frame arms the
  counter readback — `fresh_norm=false` on every probe. That was a bad instrument, not a dead
  mechanism.

  ⚠The discard is a genuine race and does NOT fire every run (0 in two later runs) — it needs a
  settle to land while a stale reading is in flight, which is why the original entry could say the
  user-visible defect was already gone. `norm_hold_break`'s heal is untouched and still covers the
  case if one ever slips through.

  Gates: selftest **173/173 + 18/18**, workspace green (app 241), **livetest 24/24 · 0 drifted**
  (so the live path is unchanged), warning-free. Offline is untouched by construction — the sinks
  are on `MandelbrotParams`, not `ExportRequest`.
...
- [ ] 🔴⭐⭐**FIELD DEVICE LOSS 2026-08-22, USER'S OWN DEEP ZOOM, beta.129 build 1837, RTX 3080,
  ...
  **motiontest PASS**, **F3 corpus 38/38**.
  ...
  ✅⭐**CLOSED 2026-09-05 — the wall-retreat decision is now a pure predicate with tests.** It had
  empirical verification (46 observed fires) and NO unit test, being inline state across frames.
  Extracted as `render::wall_shed_now(shed, throttled, acc_ms, lethal_ms)`, called from the one
  site, with `render/wall_shed.rs` beside it — 6 cases: the field frame's own 1216 ms, the `>=`
  boundary, the latch (a long pass sheds ONCE), ⚠⚠the **present-throttle veto** (the accumulator
  measures the COMPOSITOR, so shedding on it floors licences off an IDLE card — the 2026-09-04
  field log's lines, and the same poisoning the dual-view Julia saga turned on), garbage
  accumulators, and that the threshold is a PARAMETER (`--set TDR_LETHAL_MS=1` is how the harness
  forces 46 fires — a hardcoded 900 would make that run vacuous).
  ⭐**Each guard verified individually RED** by mutation: dropping the veto, dropping the latch and
  hardcoding the threshold each break exactly ONE test.
  ⭐**And the extraction itself found a flaw**: written first with an `is_finite()` guard, which
  ALSO rejected `+∞` — the most lethal reading there is, and precisely what should fire. IEEE
  comparison alone already excludes NaN and negatives, so the guard was both redundant and wrong;
  removing it also restored exact parity with the original inline rule.
...
- [x] ✅**CLOSED 2026-09-09 — the PERF warnings are an artefact of the per-call WGSL compile (mechanism
  measured below), and the report now prints the scale + fixed-cost shape itself; the one follow-up
  is split out as its own item right after this entry.** Original: 🟡**`--bench-matrix`'s PERF half cries wolf on this box — the blessed baseline no longer
  ...
  ⛔⭐⭐**MEASURED 2026-09-05 — THE PREMISE ABOVE IS WRONG, AND RE-BLESSING WOULD HAVE ERASED THE
  EVIDENCE.** "The MAGNITUDE stays uniform" is measured FALSE. Two full runs, 28 segments, plus a
  direct build A/B. Ratios against the blessed baseline run **0.97× to 2.42×** — anything but
  uniform — and the shape is systematic:

  | baseline cost | n | median ratio | median delta |
  |---|---|---|---|
  | **< 30 ms** (cheap) | 19 | **1.59×** | **+9.5 ms** |
  | **≥ 60 ms** (dear) | 7 | **1.00×** | **+0.2 ms** |

  Least-squares over all 28: **`now = 0.99 × base + 9.4 ms`**. The SCALE is 1.00 — there is no
  machine-wide slowdown at all — and what there is instead is a roughly FIXED per-segment cost that
  only shows where the GPU work is short. `floatexp-1e30-nosa/sa/nobla` (109–169 ms) come back at
  **0.97–0.99×**, i.e. unchanged; `deep-interior-1e148` measures 502–516 against a baseline of
  504.1, so the entry's own headline example ("332 → 477–493") no longer holds either — that
  number predates the current baseline file.

  ✅**NOT the palette LUT (beta.23), checked directly rather than assumed.** Built `c64d5c3` (the
  commit BEFORE the LUT) and ran the same matrix: **13 slower vs 14**, and per-segment
  post/pre median **0.991×**, mean 1.004×. The two builds are the same speed; both are slow the
  same way against the baseline.

  ⭐**And the "SET varies run to run" is just NOISE ON CHEAP SEGMENTS**: between those two
  perf-identical builds, six segments moved by more than 12% — in BOTH directions, −18% to +24%.
  At ~16 ms a segment, that is the measurement, not the renderer.

  ⛔**So do NOT re-bless.** The earlier reading ("stale baseline, re-bless or record the offset")
  would have destroyed the only record of a real, unexplained fixed cost on short frames. ⚠This is
  the same SHAPE as the open item "Offline render path: the fixed cost in the shallow/mid band" —
  worth treating as one question. ⚠A median-offset rule in the tool would be actively WRONG here:
  it would divide out the very thing that needs explaining.
  ✅The ALGORITHMIC half remains clean and trustworthy: **0 drift on both builds**, which is the
  exit-code criterion.

  ✅⭐⭐**MECHANISM FOUND, same session: it is the PER-CALL WGSL COMPILE.** `render_ms` is wall time
  around `render_export`, and **every `render_export` call builds the shader module afresh** —
  `shader_module(device)` appears at SEVEN sites in `export.rs`. Timed directly on this box:
  **9.82 ms and 8.88 ms** for the two compiles a single `--render` performs.

  **Against a least-squares fixed cost of +9.4 ms.** That is not a correlation, it is the same
  number: a fixed ~9 ms compile explains BOTH ends of the data at once — +59% on a 16 ms segment
  and +1.8% (inside noise) on the 504 ms one. The mystery is closed.

  ⭐⭐**And it is THIS REPO'S OWN RECORDED LESSON, a second time.** beta.128 hoisted the per-call
  compile out of the correction loop and wrote down why the earlier refutation had expired: *"a cost
  measured as a FRACTION expires the moment its denominator moves"* — 12 ms of a 138 ms pass was 8%
  and dismissed; once the pass became 3 ms the same 11 ms was the largest cost in the loop. The
  hoist was applied to the GATHER loop only. `render_export` itself still compiles per call, the
  WGSL has grown to **134 KB**, and the blessed baseline predates that growth.

  ⇒**The perf warnings are an ARTEFACT, not a renderer regression** — which is what "record the
  offset so a reader knows" was asking for, now with a mechanism instead of an offset.

  ▶**Follow-up worth doing, filed rather than done**: cache the `ShaderModule` per device so an
  export does not recompile 134 KB of WGSL every call. ⚠Not attempted here because a global cache
  has to be keyed by device identity and must not outlive the device — that is a real lifetime
  question, not a 10-line change, and it deserves its own commit and gates. ⭐The payoff is not the
  benchmark: a TOUR renders hundreds of frames through this path, so it is ~9–18 ms × N.

  ✅**THE REPORT NOW SAYS THIS ITSELF (2026-09-05)** — `--bench-matrix` fits
  `now = scale × baseline + fixed` over every comparable segment and prints the shape:

  ```
  shape: now ≈ 0.93× baseline + 10.2 ms fixed, over 28 segments
         a FIXED 10 ms/segment dominates the cheap segments and vanishes on the dear ones —
         read the warnings above as that one cost, not as N regressions.
  ```

  ⭐**Note `scale = 0.93`: with the fixed cost separated out the renderer is ~7% FASTER per unit of
  work than the baseline.** "11 slower" was stating the opposite of the truth. That is the whole
  argument for splitting the two terms — a list of percentages cannot, because one fixed cost reads
  as +62% on a 15 ms segment and +1.9% on a 504 ms one.
  ⚠**It REPORTS, it does not subtract.** Masking a fixed cost would hide a NEW one appearing, and
  the per-segment warnings are untouched. ⚠The diagnostic stays silent on a healthy run (|fixed| ≤
  3 ms and |scale−1| ≤ 10%) — a line that printed every time would be the same problem again.
  `scale_and_fixed` is a pure function with `bench_matrix/fit_tests.rs` beside it: a pure fixed
  cost, a pure scale, both together, an unchanged run, and degenerate input.

- [ ] **Cache the export `ShaderModule` per device** (split out 2026-09-09 from the closed
  bench-matrix entry above, where it was "filed rather than done"). Every `render_export` call
  compiles the 134 KB WGSL afresh — `shader_module(device)` at seven sites in `export.rs`, timed at
  9.82 + 8.88 ms per `--render` — which is the `+9.4 ms` fixed cost the bench-matrix fit reports, and
  ~9–18 ms × N on a tour render. ⚠Not a 10-line change: a cache must be keyed by device identity and
  must not outlive the device, so it wants its own commit and gates. beta.128 hoisted the compile
  for the GATHER loop only; this is the same lesson one level up.
...
- [ ] **WATCH — was "PRIORITY", re-labelled 2026-09-09: unexplained access violation, `08-07 17:00:25
  0xc0000005`.** One occurrence (2026-08-07), none reported since; (c) above shipped in beta.43, so a
  recurrence now produces a report on the next launch. Nothing is actionable until then. Original:
  No crash report and never investigated. Distinct from the abort class above (a segfault, not a controlled exit), so
...
- [x] ✅**DONE — this is the Render Script dialog (🎬, `ui/tour_render.rs`): a chosen script renders as
      a child `--render-tour` process with a progress bar, Cancel (kills the child) and a finish
      sound; found stale 2026-09-09.** Original: **Start an offline render from the UI** — a dialog/button to kick off a headless
...
- [x] ✅**DONE v0.2.41-beta.24–27 (found stale 2026-09-09): `.map` (beta.24), `.ugr` (beta.25), `.ggr`
  (beta.26) and `.ase` (beta.27) import into the gradient editor, plus the "found it on the web" paste
  parser (`parse_palette_text`: hex or 0–255 triples, any separators). The live entry is the ▶ PALETTE
  IMPORT item under Open bugs. Not built: image-strip sampling, which this entry itself made
  conditional ("if it stays cheap").** Original: **Palette import from other sources** (scope broadened per user 2026-08-13; was "KF `.map`
...
- [ ] **Ecosystem/platform** — no plugin system; ~~Windows-only release builds~~ Windows + Linux
  packages ship since v0.2.41-beta.20 (four: plain + accelerated per OS; no macOS — the code is
  portable wgpu/egui, a macOS job is release.yml work; stale half corrected 2026-09-09); KF interop
  partial (.kfr import yes, .kfb map files no).
- [x] ✅**DONE — `release.yml` has the `linux` job (ubuntu-22.04 tar.gz + sha256, first proven on
  v0.2.40-beta.104) and v0.2.41-beta.20 shipped four packages including `linux-accel` (ubuntu-24.04,
  GMP ≥ 6.3); found stale 2026-09-09. Real-hardware validation stays under triage B6.** Original:
...
- [ ] **WYSIWYG coordinate-expression editor** (user, 2026-09-11). The Go-to fields take the
  full grammar (Help ▸ "Coordinate expressions", beta.101, lists every constant and function
  with examples) and a refusal now names the field, the character and what to type instead —
  but the expression is still typed blind. Build a structured editor for the centre: a typeset
  preview of the expression (fractions, powers, √, π) that updates as you type, with the
  evaluated decimal beside it at the current depth's precision; a palette of the constants and
  functions rendered from `fractadyne_core::EXPR_FUNCTIONS` / `EXPR_CONSTANTS` (so it cannot
  drift from the evaluator) that inserts a signature template with the argument selected; a live
  error caret at `ExprError.pos` under the offending character; and a "kept as an expression"
  indicator showing whether the entry will be preserved and re-derived at depth
  (`is_decimal_literal` decides). Shape: a third mode in Go to location beside `x, y` and
  `offset + r ∠ θ`, or an expand-arrow on the Re/Im fields. ⚠No new grammar — the editor
  composes and edits the text the ONE evaluator reads, exactly as polar mode does; the
  deliverable is discoverability and feedback, not a second language. egui pieces: a `TextEdit`
  with a custom layouter (error highlight, caret) and a small toolbar; nothing else depends on it.
...
- [ ] **Landmark library expansion** (P1, §4.3 partial) — we ship 12 curated points; add the
  ...
  their stated (k,p).
- [x] ✅**SHIPPED v0.2.41-beta.24 (`import.rs`; editor "Import .map…" + *Hard bands*; `--palette-map`
  / `--palette-map-smooth`); found stale 2026-09-09. The verification bar below (KF-rendered
  references) is the still-OPEN §7 half of the ▶ PALETTE IMPORT item under Open bugs — tracked there,
  not here.** Original: **KF `.map` palette import** (P1, §2.6 gap) — plain 256-entry RGB text; import into the
...
- [x] ✅**SUPERSEDED (found stale 2026-09-09) by the ▶ PARAMETRIC GRADIENT EDITING item under Open
  bugs: per-segment blend curve, midpoint and colour space (RGB, HSV either way round the wheel)
  are editable in the segment-native editor (P1–P3′, beta.29–51) with the P4′ Bézier kind queued
  there; alpha is declined (the renderer has no alpha channel).** Original:
...
- [ ] **Better single-GPU utilization** — before adding GPUs, check the live dispatch actually
  ...
  ✅**MEASURED 2026-09-17 (RTX 3080 · Vulkan · build 3273).** The check is done; the three named
  levers are answered, and the 1.5–2× is real but sits somewhere else entirely.

  ⛔**FIRST, THE INSTRUMENT WAS BROKEN — every "Gsteps/s" figure ever printed was flattering.**
  `--profile`'s `gpu-it` column read the timestamped `export.iter_pass`, but on a CHUNKED tile that
  pass runs only `fs_resolve`; the actual iteration work is in `export.tilechunk_pass`, which has
  `timestamp_writes: None`. Measured 0.14 ms reported against a ~90 ms iterate — 600× under. Fixed
  by accumulating each chunk's wall into `timing::accumulate` beside `*wall_sum_ms += wall` in
  `run_tile`. ⭐Nothing below could be measured until this was repaired; re-read any older GPU-rate
  number in this file with that in mind.

  **Method.** All rows sit at the centre of the main cardioid, where every pixel is interior and
  runs the full count, so nominal work == real work and iterations/second means something. SA off
  (a throwaway `session.toml` with `series_approx = false`) and `--no-bla`, because at an interior
  centre SA skips 19,999 of 20,000 iterations and there is nothing left to measure. Every rate is a
  DIFFERENCE of two rows that differ only in `max_iter`, so the harness's fixed per-call cost
  (pipeline build, buffer alloc, full readback — `render_iter` pays all three, the live path pays
  none) cancels exactly.

  **1. Cost of one iteration, by arithmetic mode** (1024², marginal of the 20k and 10k rows):

  | mode | kernel | Giter/s | lane-instructions/iteration |
  |---|---|---|---|
  | 1 | Direct df32 (shallow, no reference) | 234 | 64 |
  | 0 | df32 perturbation @1e6 and @1e20 | 39 | 383 |
  | 2 | floatexp perturbation @1e40 | 15.6 | 955 |

  A perturbation iteration costs **6.0× a direct one**; floatexp costs **15×**. The instruction
  column is 8704 lanes × 1.71 GHz = 1.49e13 lane-instructions/s divided by the rate, and it is not
  a mystery number: hand-counting the WGSL error-free transforms (`df_add` 20 ops, `df_mul` 9,
  `df_sub` 22) gives ≈291 ops for `dz ← 2·Z·dz + dz² + dc` plus ≈78 for the df32 escape test, which
  lands on the measured 383. ⭐**The kernel is ALU-bound and the ALU is doing exactly what the
  shader asks.** It is not memory-bound: all lanes read the same reference index in lockstep, so
  the orbit fetch is a warp-uniform broadcast from a buffer that fits in L1.

  **2. Occupancy — the deep kernel saturates at ~262,000 pixels** (mode 0 @1e20, marginal µs per
  pixel per 2000 iterations; the shallow Direct kernel has a different mix and only reaches ~90% by
  262k, full rate near 1M, so its curve does NOT transfer):

  | pixels | µs/px | × the saturated cost |
  |---|---|---|
  | 16,384 (128²) | 0.135 | 2.57 |
  | 65,536 (256²) | 0.082 | 1.57 |
  | 262,144 (512²) | 0.0525 | 1.00 |
  | 1,048,576 (1024²) | 0.0544 | 1.04 |
  | 4,194,304 (2048²) | 0.0551 | 1.05 |

  **3. The color pass is 0.03–0.25 ms against 30–500 ms of iterate.** Overlapping it with async
  compute is worth ~0.1%. ⛔That lever is dead; do not spend time on it.

  ⭐⭐**THE FINDING THAT MATTERS: the live deep frame is dispatched INSIDE the under-occupied
  regime.** `--divetest tours/deep-minibrot-dive.toml` at the 1e20 window, 1280×720: p50 88.1 ms,
  p95 100.4 ms, `build95` 0.1 ms, `gpu95` 102.9 ms, motion res pinned at its 0.30 floor both ends.
  That is 384×216 = **82,944 pixels**, a third of the saturation point, paying ≈1.5× the saturated
  per-pixel cost — and with `build95` at 0.1 ms the frame is essentially 100% GPU, so there is no
  CPU side to blame.

  ⛔**The resolution controller assumes cost ∝ pixels, and that is FALSE below ~262k px** — exactly
  where a deep dive spends all of its time. Below the knee, halving the pixel count saves well
  under half the time, so resolution is a progressively worse actuator the further it is cut. Same
  frame, stated as a trade: 1280×720 at res 0.30 is 83k px; at res 0.56 it is 289k px — **3.5× the
  pixels for about 2.3× the time.** Detail per millisecond improves by ~50% by rendering MORE.

  **Ranked follow-ups, by measured leverage:**
  1. ⭐Teach the motion-res controller the knee. It need not know the constant: the AIMD already
     measures wall time, so give it the pixel count as a second input and stop it cutting below the
     point where cutting stops paying. `min_motion_res` is a fraction of the window, which is the
     wrong unit — the knee is an absolute pixel count. Connects to the low-zoom detail-loss report
     ⇒ `design/live-zoom-smoothing.md` §5.
  2. Skipping beats cheapening: SA and BLA are worth ~30× at an interior 1e20 view (576 ms → 19 ms
     for the same frame). Any work on acceleration COVERAGE outranks any work on the inner loop.
  3. The df32 escape test is ~78 of 383 ops (20%) and runs in full double-float every iteration
     when an f32 estimate would decide a bailout-radius comparison. ~15–20%, and self-contained.
  4. ⛔**A second GPU is now the honest path to 2×, not the last resort this item assumed.** The
     one device is saturated at realistic frame sizes and the kernel is issue-bound with no slack,
     so the cheap single-GPU win this item hoped for does not exist. See the multi-GPU item below.

  **Also learned, offline path:** `export_tile_cap`'s `cap` starts AT the nominal bound and is
  clamped to it as the ceiling, so it can only halve when hot and recover — a purely downward
  safety mechanism, working as designed (an earlier reading of this session that a 2048² export ran
  ~237 tiles was WRONG: at the default 2e10 budget and 20k iterations the bound is 1000 px, so it
  runs 9). Separately, the tiled+chunked export path costs ~25% more than one unchunked dispatch
  for identical work (2048²×20k: 397–412 ms unchunked vs 499–515 ms of accumulated chunk wall) —
  the price of staying under the watchdog, worth knowing but not worth removing.

  Region files and raw logs: `sat-mode.toml`, `sat-deepsize.toml`, `sat-size.toml`, `sat-fixed.toml`.

- [x] ✅**`find_nucleus` returned one atom's PERIOD with another atom's CENTRE** — fixed
  2026-09-17, `--selftest` back to **190/190 + goldens 19/19**.

  The symptom was the NR-zoom "deep minibrot: size, framing, center accuracy" check reporting
  **period 973** at the seahorse seed viewed at 1e6×, where it wants 998 (`size 2^-48.242,
  moved 1.8e-9` against a threshold of `period 998, size 2^-50.5`).

  ⚠**Two wrong turns worth not repeating.** First I called it a regression against the committed
  `validation/report.md` — but that file was last regenerated at **beta.71** (`31c2646`), roughly
  40 betas ago, so it is a stale artifact and not a baseline. Second I assumed a precision problem
  and built a retry loop that re-solved at a precision derived from the atom size; it changed
  nothing, because tracing showed `detect_period` returns **998 at every precision from 70 to 179
  bits**. The detector was never wrong. ⭐The trace that found it took five minutes and replaced an
  hour of plausible theory — instrument before theorising.

  **Root cause: `reduce_period` scanned every `n ≤ p_est` for the first one within tolerance.**
  The reduction exists only because `detect_period` may return an integer MULTIPLE of the true
  period, and the fact that licenses it — `Z_m = 0 ⟹ Z_{jm} = 0` — runs one way. Scanning every
  `n` asks a different question ("is ANY lower-period nucleus near this point"), and near a deep
  atom something always is. So Newton converged correctly on the 998 nucleus and the reduction
  then relabelled it 973 — a real neighbouring minibrot's period pinned to the 998 minibrot's
  centre. A jump there lands ~500,000 atom-widths into empty space, which is exactly what the
  check's `moved` column was reporting. The tolerance is a thousandth of the VIEW span, so it
  loosened as the view widened: measured 39 at 1e1.8×, 468 at 1e3.9×, 858 at 1e4.8×, 973 at 1e6×,
  and only 998 once the view reached ~1e6.6× — the "nearest minibrot" answer depended on how far
  out the user happened to be standing.

  **Fix**: `reduce_period` now considers only divisors of `p_est` (one `continue`). Every view from
  1e1.8× to 1e12× returns period 998, size 2^-50.5046, residual 2^-194, and the 1e6× row reproduces
  the values beta.71's report recorded to the last digit (`moved 7.0e-31`). Pinned by a new
  `tests/nucleus_outcomes.rs` case that sweeps the view magnification rather than fixing one — the
  answer must not depend on the view — and asserts the period, the atom size, and that re-refining
  barely moves the centre.

- [ ] **`about_names_the_running_backend` is not isolated from the rest of its test binary** —
  the test asserts the About panel's arithmetic line honestly reads `none (no reference orbit built
  yet)`, which is the right assertion: the stamp must report what RAN, never what the build
  contains. But the stamp is process-global and the app crate's unit tests share one binary, so any
  sibling test that finishes a reference orbit first sets it and this one fails. Seen once on
  2026-09-17 (392/1) and green on the three runs either side of it, alone and in full, so it is
  ordering, not code. ⛔Do not fix it by loosening the assertion — that would re-admit exactly the
  "reads configuration instead of observation" failure the mask exists to prevent. Give the test its
  own binary, or let it observe and restore the stamp.

- [ ] 🔴**"Normalize deep colors" GETS NO READINGS AT FLOATEXP DEPTH, so it cannot engage on load**
  — REPRODUCED 2026-09-17, not a mechanism argument. `--soak 18 --soak-depth N` with
  `FRACTADYNE_TRACE=gpu`, counting `norm range` lines in the log:

  | depth | norm-range readings | tiling |
  |---|---|---|
  | 1e12 | 8 | `tile=None`, chunked |
  | 1e20 | 7 | `tile=None`, chunked |
  | **1e51** | **0** | `tile=None`, chunked |

  1,411 frames at 1e51 over 25 s produced not one reading, so `norm_range` and `norm_grad` stay
  `None`, `live_norm_cycle_offset` returns `None`, and the view paints unnormalized however the
  checkbox is set. ⭐**That is the user's report exactly**: "on app load the colors aren't
  normalized; tweaking the view causes it to recalculate and may cause it to normalize" — a nudge
  puts the view on a path that does produce a reading, and it sticks.

  ⛔**Two suspects RULED OUT, so nobody re-checks them.** (1) Tiling: `arm_ctr` requires
  `tile.is_none()`, which looked decisive, but the tile trace shows `tile=None` and `tiling=false`
  at ALL THREE depths — every one of them is chunked, not tiled. (2) The shader not writing the
  counters in mode 2: `resolve_fe_pipeline` is the SAME `fs_resolve` entry point as the df32 path,
  and it does `atomicAdd` to `CTR_GRAD_SUM`/`CTR_GRAD_N`/`CTR_MAXITER`.

  ⭐What is left is the arming/readback side at floatexp: `arm_ctr`'s third term is
  `view.counter_read.state == TimingState::Idle`, so a readback that never drains arms exactly
  once and then never again — which fits a count of ZERO only if it never armed at all, so check
  the ordering of the mode-2 chunk passes against the clear and the copy first. One observable
  difference at 1e51 is that the chunk walk reads `walk=[0,37392)` with `cur` already at the ask
  every frame, where 1e20 shows a normal advancing `walk=[6987,14737)`.

- [ ] 🔴**`--uitest` step 43 `minimap-pan-verify` is a TIMING RACE, not a flake to wave through**
  — "panning from the minimap left a stale frame: meanD 9.9/10.5/16.8 against an honest re-render
  of the SAME view". Measured 2026-09-17: **0 passes in 7 runs** with `timing::accumulate` in
  `run_tile`, **3 passes in 4 runs** without it. ⛔That correlation is NOT a causal finding and must
  not be filed as one: `timing::accumulate` writes a thread-local `Cell` whose only readers in the
  whole workspace are `livetest.rs` and `profile.rs`, neither of which runs under `--uitest`
  (grepped). The believable reading is that `run_tile` sits on the live tiled-settle path
  (`render_iter_tiled`, `render.rs`), one more per-chunk write nudges an already-marginal settle
  race, and the odds move. ⭐**The defect is the race.** Find what makes a minimap pan present
  before its re-render lands, rather than tuning whatever currently happens to win it. Step 32
  (`live-floatexp-1e30` flat) warns on every run of both arms and is the known 3080 floatexp issue
  ⇒ `topic-uitest`; a run that passes it is the anomaly, not the rule.
...
## ▶ Render the ultradeep view34 location (someday) (2026-09-12)

- [ ] ⭐**Attempt a Fractadyne render of an ultradeep minibrot shared by the FractalShark author.**
  A ~650,000-digit centre at **1e650452 zoom** (period **399289978**), found by a months-long
  Newton-Raphson zoom. The coordinates, reference images and the author's technical notes are in
  `local/views/` (`view34_coords.txt` / `.zip`, plus `view34-NOTES.md`; not committed — 3.9 MB and
  `local/` is gitignored). The author shared this location publicly on fractalforums. ⚠Far past our
  deepest tracked location (`validation/spiral-9.98e60205.fdn`, 60,231
  digits): expect a very long arbitrary-precision reference build. Plan: import the coordinate and
  verify precision handling first, then a low-res live view before attempting any export.
