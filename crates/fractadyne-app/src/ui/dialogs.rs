...
impl FractadyneApp {
    ...
    fn build_dive_script(&self, note: &str, secs: f64) -> String {
        ...
        // Target magnification line — `mag_log10` past f64's ~1e308 ceiling, plain `mag` below it.
        let target_mag_line = if log10mag < 300.0 {
            format!("mag = {target_mag}")
        } else {
        ...
        // Phase 1 — the whole set (classic framing).
        ...
        let total = if deep {
            ...
            s.push_str("mag = 8.0\n");
            s.push_str(&format!("secs = {SWOOP_SECS}\n"));
            s.push_str("ease = \"smooth\"\n");
            s.push_str("hold = 0.5\n\n");
            // Phase 3 — the dive. Center OMITTED ⇒ inherited ⇒ held fixed on the point.
