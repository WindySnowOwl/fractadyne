//! `--reusetest`: headless staleness harness for the reuse-first zoom (design/xaos-reuse.md,
//! Stage 0). For each view it renders a from-scratch reference, then for a series of Δ-octave
//! zoom-ins renders the true (from-scratch) zoomed frame AND the *reprojection* the live path
//! would show for that frame — the exact color-pass affine magnify of the reference — and reports
//! how far the reprojection has drifted from the truth as a function of Δ.
//!
//! Why: the live path holds+reprojects the frozen frame for `REFRESH_OCTAVES = 0.5` octaves of a
//! dive before taking one real re-iterate (render.rs). That 0.5 was a guess; this measures the
//! reprojection error vs Δ so the cadence can be chosen from data, and it is the from-scratch
//! golden foundation the fuller reuse work needs. Touches no live-loop state (renders isolated
//! `render_iter` frames), so it is safe and deterministic.

use crate::{ColorMethod, FractadyneApp, FractalKind};
use fractadyne_core::{parse_bf, precision_for_magnification, FloatExp, Viewport};

struct ReuseView {
    name: &'static str,
    cx: &'static str,
    cy: &'static str,
    mag: f64,
}

impl FractadyneApp {
    pub(crate) fn run_reusetest(
        &mut self,
        device: &eframe::wgpu::Device,
        queue: &eframe::wgpu::Queue,
    ) {
        const N: u32 = 512;
        // Fixed iteration count across every render so the comparison isolates the *spatial*
        // reprojection error, not auto-iter count differences between the base and zoomed views.
        const ITER: u32 = 4000;
        // Hermetic config (mirrors the selftest baseline): the harness must not depend on the
        // live session. `--reusetest` exits via process::exit, so no restore is needed.
        self.fractal = FractalKind::Mandelbrot;
        self.julia_mode = false;
        self.dual = false;
        self.render_cfg.auto_iter = false;
        self.render_cfg.max_iter = ITER;
        self.render_cfg.series_approx = true;
        self.render_cfg.use_bla = true;
        self.render_cfg.glitch_correct = false;
        self.coloring.color_method = ColorMethod::Smooth;

        let views = [
            ReuseView { name: "seahorse-1e6", cx: "-0.743643887037151", cy: "0.131825904205330", mag: 1.0e6 },
            ReuseView { name: "seahorse-1e12", cx: "-0.743643887037151", cy: "0.131825904205330", mag: 1.0e12 },
            // Precise boundary point (the golden "mandelbrot-1e6"): richly escaping, so the
            // escaped-pixel comparison is well-populated (unlike a coarse-precision centre).
            ReuseView {
                name: "boundary-1e6",
                cx: "-7.219621882920463979621343199249635039400777157391994056859e-1",
                cy: "2.406540627640154659873781066416545013133592385797331352286e-1",
                mag: 1.0e6,
            },
        ];
        let deltas = [0.1_f64, 0.25, 0.5, 0.75, 1.0];

        // render_iter for a (center, mag) view → RGBA32F pixels (channel 0 = smooth iter, <0 = interior).
        let render = |app: &Self, cx: &str, cy: &str, mag: f64| -> Option<Vec<f32>> {
            let mut vp = Viewport::new(N as f64, N as f64);
            vp.center_x = parse_bf(cx)?;
            vp.center_y = parse_bf(cy)?;
            vp.units_per_pixel = FloatExp::from_f64(3.0 / (N as f64 * mag));
            vp.precision = precision_for_magnification(mag);
            let mut req = app.current_export_request_for(&vp, false);
            req.width = N;
            req.height = N;
            req.ss = 1;
            fractadyne_gpu::render_iter(device, queue, &req).ok().map(|r| r.pixels)
        };

        println!(
            "reuse-first zoom staleness — reprojection vs from-scratch, {N}×{N}, iter {ITER} (fixed).\n\
             `reprojection` = the exact color-pass affine magnify (uv_scale = 2^-Δ about centre) of the\n\
             base frame, i.e. what the live view shows while holding through a Δ-octave dive.\n"
        );
        println!(
            "  {:<16} {:>5}  {:>7}  {:>10}  {:>9}  {:>9}",
            "view", "Δoct", "%escap", "meanΔiter", "maxΔiter", "%>2iter"
        );
        println!("  {}", "-".repeat(64));

        let nn = N as usize;
        for v in &views {
            let Some(base) = render(self, v.cx, v.cy, v.mag) else {
                println!("  {:<16}  (render failed)", v.name);
                continue;
            };
            for &d in &deltas {
                let magd = v.mag * 2f64.powf(d);
                let Some(fresh) = render(self, v.cx, v.cy, magd) else { continue };
                // The zoomed-view pixel (x,y) samples the base frame at the magnified coordinate
                // (nearest, matching the shader's `pix = i32(suv * screen_dim)`): a Δ-octave
                // zoom-in is a 2^Δ magnification about the centre, so uv_scale = 2^-Δ.
                let s = 2f64.powf(-d);
                let (mut sum, mut n, mut big) = (0.0f64, 0u64, 0u64);
                let mut maxe = 0.0f64;
                for y in 0..nn {
                    for x in 0..nn {
                        let uvx = (x as f64 + 0.5) / N as f64;
                        let uvy = (y as f64 + 0.5) / N as f64;
                        let bx = (((uvx - 0.5) * s + 0.5) * N as f64) as i64;
                        let by = (((uvy - 0.5) * s + 0.5) * N as f64) as i64;
                        let bx = bx.clamp(0, N as i64 - 1) as usize;
                        let by = by.clamp(0, N as i64 - 1) as usize;
                        let reproj = base[(by * nn + bx) * 4];
                        let truth = fresh[(y * nn + x) * 4];
                        if reproj >= 0.0 && truth >= 0.0 {
                            let e = (reproj - truth).abs() as f64;
                            sum += e;
                            n += 1;
                            if e > 2.0 {
                                big += 1;
                            }
                            if e > maxe {
                                maxe = e;
                            }
                        }
                    }
                }
                let mean = if n > 0 { sum / n as f64 } else { 0.0 };
                let pct = if n > 0 { 100.0 * big as f64 / n as f64 } else { 0.0 };
                let escap = 100.0 * n as f64 / (nn * nn) as f64;
                println!(
                    "  {:<16} {:>5.2}  {:>6.1}%  {:>10.3}  {:>9.1}  {:>8.1}%",
                    v.name, d, escap, mean, maxe, pct
                );
            }
        }
        println!(
            "\nRead: this is a CONSERVATIVE (raw smooth-iter) staleness proxy — a NEAREST reprojection\n\
             can't preserve sub-pixel escape-time bands, so in fine detail (seahorse spirals) the\n\
             per-pixel iter error is already large at small Δ and grows slowly after. Perceptual\n\
             staleness after coloring is lower (band shifts partly hide in the cycle), so this bounds\n\
             — doesn't set — REFRESH_OCTAVES (0.5, render.rs). Takeaways: (1) reprojection is a\n\
             placeholder that loses real per-pixel detail fast, which is why the live path also\n\
             periodically re-iterates and, once settled, tile-refines to native — the case for the\n\
             Stage-2 during-motion tile refine in design/xaos-reuse.md; (2) a colored-image follow-up\n\
             (sRGB diff, like the goldens) would pin the perceptual cadence."
        );
    }
}
