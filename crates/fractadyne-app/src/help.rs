...
pub(crate) const CLI_REFERENCE: &[CliRef] = {
    ...
    &[
        ...
        Flag("--gputest", "Dev: verify the shader's df32/floatexp arithmetic primitives (error-free transforms, double-float ops, floatexp ops, iteration-form accumulation) against CPU oracles on this machine's GPU/driver, and print a per-op verdict table. Catches fast-math fma contraction, flushed denormals, or wrong rounding on unfamiliar hardware — include the table in bug reports about wrong deep rendering. Exit 1 if any op family fails."),
