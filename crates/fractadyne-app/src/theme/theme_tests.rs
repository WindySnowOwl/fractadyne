...
fn theme_contrast_meets_minimum() {
    for (name, p) in [("dark", Palette::dark()), ("light", Palette::light())] {
        ...
        //
        // ⚠⚠**MEASURED, and the floor is the standard each theme actually MEETS.** The LIGHT
        // accent `#855C0C` clears the full 4.5:1 body-text floor on all seven surfaces
        // (4.60–5.94 on window / panel / surface / elevated / hover / active / selection; the
        // worst is `active`). It was deepened there from `#B98212`, which failed everywhere
        // (2.59 worst) — an accent label on a light-theme hovered or selected widget really was
        // hard to read, which is what checklist step 7 asks a human to look for. The DARK accent
        // `#E0A030` clears 3.0:1 everywhere (3.97–8.03) but not 4.5 (its worst is 3.97), and
        // deepening it would dull the one bright mark on a near-black UI — so it stays gated at
        // the 3.0 it achieves. Neither can regress unnoticed; raising the light accent back
        // toward `#B98212` now fails the gate.
        let accent_floor = if name == "light" { 4.5 } else { 3.0 };
        for (bg_name, bg) in [
            ("window", p.window),
            ("panel", p.panel),
            ("surface", p.surface),
