...
//!
//! Modularization is ongoing — next candidates: the self-test into `selftest.rs`, then the
//! render-request builders and the UI panels (stepwise, since they touch the live-render
//! path). Moving items between modules in one crate has no runtime cost (no added
